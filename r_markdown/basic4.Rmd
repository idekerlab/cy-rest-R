---
title: "Network Analysis with igraph 2"
author: "Keiichiro Ono"
date: "8/9/2014"
output: html_document
---

## Basic Workflow 4: Working with igraph: Part 2
...
## Introduction
Welcome to part 4 of the R tutorial.  In this section, you will learn more workflow using Cytoscape and igraoh
...
### Essentiality and Node Degree

#### Loading Network and Table Data into igraph
```{r}

# Build E. Coli Network from text Table
eco.net.df <- read.table("data/ecoli_ppi_Hu_etal_2009.txt")
eco.net <- simplify(graph.data.frame(eco.net.df,directed=F),remove.multiple=T,remove.loops=T)

summary(eco.net)

# View the entire network in Cytoscape
send2cy(toCytoscape(eco.net), "default", 'grid')

# Load essentiality
ess<-read.table("data/ecoli_proteins_essentiality_Baba2006MSB.txt",header=T)
head(ess)

# Extract list of target genes
targets <- intersect(ess$gene, V(eco.net)$name)
print(targets)

ess_select <- subset(ess,is.element(ess$gene,targets)==T & duplicated(ess$gene)==F)
ess_select_ord <- ess_select[order(ess_select$gene),]


deg <- degree(eco.net)
deg_select <- subset(deg,is.element(names(deg),targets)==T)
